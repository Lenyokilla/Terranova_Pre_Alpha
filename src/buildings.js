...
// Zeichnet einen isometrischen Torbogen auf einer Wandfläche
function wallArch(Pb0, Pb1, Pt0, Pt1, u0, u1, v0, vTop, fillCol, brickCol, s) {
  ...
  // Quadratische Bezier-Kurve für die Bogenrundung
  const topMid = lerp(D, C, 0.5);
  ctx.quadraticCurveTo(topMid.x, topMid.y, S1.x, S1.y);
  ctx.lineTo(B.x, B.y);
  ctx.closePath();
  ctx.fill();

  // 2. Die dekorative Ziegel-Bogenkante (Architrav/Keilsteine)
...
// Zeichnet horizontale dünne Ziegellinien (Ausgleichsschichten) quer über eine Wandseite
function wallBrickLines(Pb0, Pb1, Pt0, Pt1, floors, col, s) {
  ctx.strokeStyle = col;
  ctx.lineWidth = Math.max(0.8, 1 * s);
  // Pro Stockwerk ziehen wir 2 dünne, nahe beieinanderliegende Ziegelbänder
  for (let f = 0; f < floors; f++) {
    const fh = 1 / floors;
    const v1 = (f / floors) + fh * 0.35;
...
function drawForum(gx, gy, baseLift) {
  ...
  ctx.strokeStyle = 'rgba(60,46,26,.30)'; ctx.lineWidth = 1;
  ...
  ctx.fillStyle = stone; poly([b1.Nt, b1.Et, b1.St, b1.Wt]);
  ...
  ctx.fillStyle = shade(marble, 0.06); poly([e3.Wt, e3.St, apex]);
  ctx.fillStyle = shade(marble, -0.10); poly([e3.St, e3.Et, apex]);
...
function drawClaypit(gx, gy, baseLift) {
  ...
  ctx.fillStyle = shade(rim, -0.10); poly([g0.W, g0.S, gr.St, gr.Wt]);
  ctx.fillStyle = shade(rim, -0.26); poly([g0.S, g0.E, gr.Et, gr.St]);
  ctx.fillStyle = rim; poly([gr.Nt, gr.Et, gr.St, gr.Wt]);
  const ctr = { x: (gr.Nt.x + gr.St.x) / 2, y: (gr.Nt.y + gr.St.y) / 2 + 2 * s };
...
function drawPottery(gx, gy, baseLift) {
  ...
  const kb = lerp(c.S, c.E, 0.5), kt = lerp(c.St, c.Et, 0.5);
  ...
  ctx.fillStyle = 'rgba(205,205,205,.45)';
...
function drawWell(gx, gy, baseLift) {
  ...
  ctx.strokeStyle = 'rgba(60,50,40,.7)'; ctx.lineWidth = 1;
...
function drawHarvester(x, y, s, seed) {
  ...
  ctx.fillStyle = '#9c8048'; ctx.beginPath(); ctx.ellipse(x, y - 4 * s, 2.4 * s, 4 * s, 0, 0, 7); ctx.fill();
  ctx.fillStyle = '#caa06a'; ctx.beginPath(); ctx.arc(x, y - 9 * s, 1.9 * s, 0, 7); ctx.fill();
  ctx.strokeStyle = '#6e4a2a'; ctx.lineWidth = Math.max(1, 1.1 * s); ctx.lineCap = 'round';
  const hx = x + Math.cos(swing) * 6 * s, hy = y - 5 * s + Math.sin(swing) * 4 * s;
  ctx.beginPath(); ctx.moveTo(x, y - 5 * s); ctx.lineTo(hx, hy); ctx.stroke();
  ctx.strokeStyle = '#c9c4ba'; ctx.beginPath(); ctx.arc(hx, hy, 3 * s, swing - 0.4, swing + 1.4); ctx.stroke();
  ctx.lineCap = 'butt';
}
...
function drawGrainfield(gx, gy, baseLift) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    const cut = stage === 'harvest' && (i / N) < hp;
    if (cut) {
    ...
    const gg = ripe ? 1 : g;
    const hgt = (2 + gg * 9) * s;
    const cf = ripe ? 1 : Math.max(0, (gg - 0.5) / 0.5);
    const col = mixHex('#5aa83f', '#caa53e', cf);
...
// Römisches Mietshaus (Insula): hoher Putzbau mit Ziegel-Ausgleichsschichten, Torbögen im Erdgeschoss
function drawInsula(gx, gy, lvl, baseLift) {
  ...
  // Geschoss-Gesims Linien
  for (let f = 1; f < floors; f++) {
    const b0 = f / floors;
    const swA = lerp(c.W, c.Wt, b0), swB = lerp(c.S, c.St, b0), seB = lerp(c.E, c.Et, b0);
    ctx.strokeStyle = 'rgba(40,28,14,.20)'; ctx.lineWidth = Math.max(1, 1 * s);
  ...
  if (floors >= 2) {
    ...
    ctx.fillStyle = roofCol; poly([r.Nt, r.Et, r.St, r.Wt]);
    ctx.strokeStyle = shade(roofCol, -0.18); ctx.lineWidth = Math.max(1, 0.8 * s);
  ...
  // --- NEU: DÜNNE HORIZONTALE ZIEGEL-AUSGLEICHSSCHICHTEN ÜBER DEN PUTZ ---
  const brickColSW = 'rgba(130, 65, 45, 0.42)', brickColSE = 'rgba(95, 45, 30, 0.48)';
  wallBrickLines(c.W, c.S, c.Wt, c.St, floors, brickColSW, s);
  wallBrickLines(c.S, c.E, c.St, c.Et, floors, brickColSE, s);

  // --- RENDER FENSTER & ERDGESCHOSS-TORBÖGEN (FORNICES) ---
  ...
  for (let f = 0; f < floors; f++) {
    ...
    if (f === 0) {
      ...
      // Höhere Stockwerke: Normale, rechteckige Fensterreihen
...
function drawFirehouse(gx, gy, baseLift) {
  ...
  ctx.fillStyle = shade(wall, -0.08); poly([c.W, c.S, c.St, c.Wt]);
  ctx.fillStyle = shade(wall, -0.28); poly([c.S, c.E, c.Et, c.St]);
  wallPatch(c.W, c.S, c.Wt, c.St, 0.38, 0.62, 0.05, 0.62, '#7a2e1e');
  wallPatch(c.S, c.E, c.St, c.Et, 0.2, 0.4, 0.45, 0.7, '#2b231c');
  ...
  ctx.fillStyle = '#5a3d22'; ctx.fillRect(r.Et.x - 3 * s, r.Et.y - 2 * s, 3 * s, 3 * s);
  return { cx: c.cx, topY: r.Nt.y };
}
...
function drawEngineer(gx, gy, baseLift) {
  ...
  ctx.fillStyle = shade(wall, -0.08); poly([c.W, c.S, c.St, c.Wt]);
  ctx.fillStyle = shade(wall, -0.28); poly([c.S, c.E, c.Et, c.St]);
  wallPatch(c.W, c.S, c.Wt, c.St, 0.4, 0.6, 0.05, 0.6, '#3a2f24');
  ...
  ctx.beginPath(); ctx.moveTo(p0.x, p0.y); ctx.lineTo(u0.x, u0.y - 4 * s); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(p1.x, p1.y); ctx.lineTo(u1.x, u1.y - 4 * s); ctx.stroke();
  for (const v of [0.4, 0.75]) { const a = lerp(p0, u0, v), b = lerp(p1, u1, v); ctx.beginPath(); ctx.moveTo(a.x, a.y - 2 * s); ctx.lineTo(b.x, b.y - 2 * s); ctx.stroke(); }
  ctx.beginPath(); ctx.moveTo(p0.x, p0.y); ctx.lineTo(u1.x, u1.y - 6 * s); ctx.stroke();
  ctx.lineCap = 'butt';
...
function drawBuilding(gx, gy, kind, lvl, baseLift, statusEffects) {
  ...
  if (windows) {
    ...
    drawTier(0.15, 0.42);
    if (lvl >= 2) drawTier(0.55, 0.78);
    if (lvl === 3) drawTier(0.84, 0.96);
  }
  ...
  if (statusEffects.plagueRisk && kind === 'house') {
    ctx.fillStyle = 'rgba(80, 110, 60, 0.25)';
