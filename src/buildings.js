...
function drawForum(gx, gy, baseLift) {
  ...
  // 2. Podium (Basis des Tempels/Forums) mit weichen Verläufen
  ...
  column(b1.St, colH); // Monumentale Ecksäule im Vordergrund

  // 4. Gebälk (Entablement)
  ...
  // Ziegelbänder am Gebälk-Architrav
  ...
  // 5. Flaches römisches Satteldach mit Tympanon (Giebeldreieck)
  const apex = { x: (e3.Nt.x + e3.Et.x + e3.St.x + e3.Wt.x) / 4, y: (e3.Nt.y + e3.Et.y + e3.St.y + e3.Wt.y) / 4 - roofH * s };
  const roofCol = '#b15f3a';
  
  ctx.fillStyle = shade(roofCol, -0.35); poly([e3.Nt, e3.Wt, apex]); poly([e3.Nt, e3.Et, apex]); // Hinten
  ctx.fillStyle = shade(roofCol, 0.05); poly([e3.Wt, e3.St, apex]); // SW (heller Ziegel)
  ctx.fillStyle = shade(roofCol, -0.18); poly([e3.St, e3.Et, apex]); // SE (dunkler Ziegel)
  
  // Ziegelandeutung auf den Dachflächen
  tileFace(e3.Wt, e3.St, apex, shade(roofCol, 0.05));
  tileFace(e3.St, e3.Et, apex, shade(roofCol, -0.18));

  // Tiefenrelief im Giebeldreieck (Tympanon-Schatten)
...
function drawClaypit(gx, gy, baseLift) {
  ...
  // Römische Stützwände aus Opus Latericium um die Grube herum
...
function drawWell(gx, gy, baseLift) {
  // Ein repräsentatives römisches Brunnenhaus / Puteal aus Marmor
  ...
  // Horizontale Fugen im Marmor des Puteals
  ...
  // Holzpfosten und römisches Ziegeldach darüber
...
function drawMarket(gx, gy, baseLift) {
  // Ein Macellum (römische Markthalle/Verkaufsstand)
  ...
  // Rote/weiß-gestreifte römische Stoffmarkise (Canopy)
  const topY = canopyRoof(c, roofAwn, 10);
  return { cx: c.cx, topY };
}

function drawFirehouse(gx, gy, baseLift) {
  // Station der Vigiles (Römische Feuerwehr)
  ...
  // Löschwasser-Zisterne auf dem Dach
  const c2 = r;
...
function drawEngineer(gx, gy, baseLift) {
  // Das Bureau des Architectus (Ingenieur/Architekt)
  ...
  // Römischer hölzerner Vermessungskran oder Groma-Halterung an der Wand (Scaffolding-Deko)
...
function drawMill(gx, gy, baseLift) {
  // Römische Mühle (Getreidemühle mit großem hölzernen Antriebsrad)
