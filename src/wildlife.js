/* TERRA · wildlife.js */
// ---- Globale Tierwelt-Strukturen (Sicherheits-Initialisierung) ----
if (typeof sheep === 'undefined') var sheep = [];
if (typeof birds === 'undefined') var birds = [];
if (typeof herdAnchor === 'undefined') var herdAnchor = null;

// Neue Arrays & Anker (jetzt sicher deklariert)
var ducks = [];
var duckAnchor = null;
var fish = [];
var butterflies = [];
...
function randWater() {
  ...
  return null; // Falls kein Wasser auf der Map ist
}
  ...
  for (const s of sheep) {
    ...
    if (s.moving) {
      ...
      if (isPassableForSheep(nextX, nextY)) {
        ...
        s.moving = false;
        s.gx -= (sdx / sd) * 0.02; s.gy -= (sdy / sd) * 0.02;
      }
    }
  }
}
...
function drawSheep(s) {
  ...
  // Flüssige, geglättete Animation (kein Zittern mehr)
  let bob = 0;
  if (s.moving) {
    bob = (1 - Math.cos(animT * 2.2 + s.bob)) * 0.8 * sc;
  } else {
    bob = Math.sin(animT * 0.8 + s.bob) * 0.15 * sc;
  }
...
function initBirds() {
  ...
  for (let i = 0; i < n; i++) {
    birds.push({
      gx: bx - i * 0.5, gy: by + (Math.random() - 0.5) * 1.5,
...
function initDucks() {
  ...
  for (let i = 0; i < n; i++) {
    ducks.push({
      gx: c.x, gy: c.y,
      offx: -i * 0.4 - (Math.random() * 0.2), offy: (Math.random() - 0.5) * 0.3,
...
function initFish() {
  ...
  for (let s = 0; s < 3; s++) {
    ...
    for (let i = 0; i < numInSwarm; i++) {
      fish.push({
        gx: c.x + (Math.random() - 0.5) * 0.8, gy: c.y + (Math.random() - 0.5) * 0.8,
        cx: c.x, cy: c.y, angle: Math.random() * Math.PI * 2, sp: 0.3 + Math.random() * 0.2, swarmId: swarmId
      });
    }
  }
}
...
function initButterflies() {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    butterflies.push({
      gx: m.x + 0.5, gy: m.y + 0.5, sx: m.x + 0.5, sy: m.y + 0.5,
...
function drawAllWildlife() {
  // Reihenfolge sichert die richtige visuelle Tiefe (Fische ganz unten, Vögel ganz oben)
  drawFish();
  drawDucks();
  if (typeof sheep !== 'undefined') {
